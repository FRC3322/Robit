...
DriveForward::DriveForward(double d, double s) {
	...
	defaultSpeed = fabs(s)*(defaultDistance < 0.0 ? -1 : 1);
	speed = 0.0;
...
void DriveForward::Initialize() {
	...
	distanceRemaining = fabs(distance);
...
void DriveForward::Execute() {
	...
	distanceRemaining = fabs(distance - Robot::DistanceTraveled());
	if (distanceRemaining <= 6.0) speed *= 0.5;
	Robot::drivetrain->drive->Drive(-speed, curve);	// FIXME replace with PID
}
bool DriveForward::IsFinished() {
	return (Timer::GetPPCTimestamp() >= keepGoingUntilTime &&
			distanceRemaining <= 0.0) || IsTimedOut();
}
